class Api::RingtonesController < ApplicationController
  ...
  def create
    ...
    if @ringtone.save
      render json: { success: true, message: 'ringtone uploaded successfully', status: 201, data: @ringtone.new_attr },
             status: 201
    else
      render json: { success: false, message: 'ringtone uploaded unsuccessfully', status: 422, data: @ringtone.errors },
             status: 422
    end
  end
  ...
  def update
    if @ringtone.update(ringtone_params)
      render json: { success: true, message: 'ringtone updated successfully', status: 200, data: @ringtone.new_attr },
             status: 200
    else
      render json: { success: false, message: 'ringtone updated unsuccessfully', status: 422, data: @ringtone.errors },
             status: 422
    end
  end
