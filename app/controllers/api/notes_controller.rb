class Api::NotesController < ApplicationController
  ...
  def create
    ...
    if @note.save
      render json: { success: true, status: 201, data: @note.new_attr }, status: 201
    else
      render json: { success: false, status: 422, message: @note.errors }, status: 422
    end
  end
  ...
  def note_params
    params.require(:note).permit(:subject, :description, :event_date, :reminder_date, :ringtone_id, :note_type, :list_id)
  end
end
